...
<html lang="pl">
	...
	<body>
		...
		<form method="post">
				...
				if(!empty($_POST['tablica'])){
					if (preg_match('/^(-?[0-9],?)+\b$/',$_POST['tablica'])){
						$tablica =$_POST['tablica'];
						$tablica_danych = explode("," ,$tablica);
						echo '<h2>Wprowadzone dane:</h2>';
						print_r($tablica_danych);
						echo'<br><br>';
						echo' <b>WYNIK:</b>';
						solution($tablica_danych);
}
					else{
						echo '<h2 style="color:red;">Błąd! Wprowadzenie danych do tablicy nie może być:  '.$_POST['tablica'].'</h2>';
						echo '<h2> Monżna wprowadzać tylko cyfry przez przecinek bez spacji!
								<br>Dane także nie mogą kończyć się przecinkiem!</h2>';
}
};
		echo'</form>
